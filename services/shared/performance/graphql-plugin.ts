...
export function createPerformancePlugin(options: PerformancePluginOptions) {
  const config = loadPerformanceConfig();
  
  return {
    onRequest() {
      return {
        onRequestEnd({ context }: any) {
    ...
    onExecute() {
      return {
        onExecuteParse({ args }: any) {
          const context = args.contextValue;
          const monitor = new PerformanceMonitor(
            options.eventBus || context.eventBus,
            options.logger || context.logger,
            options.slowThreshold || config.thresholds.slowOperationMs,
            config
          );
...
function countFields(document: any): number {
  let count = 0;
