 ...
 */
export function buildCosmoSubgraphSchema(config: SubgraphConfig): GraphQLSchema {
...
export function createReferenceResolver<T>(
  fetchFn: (id: string, context: any) => Promise<T | null>
) {
  return async (reference: EntityReference, context: any) => {
