...
async function start() {
  console.log('Starting Advanced Mesh Gateway with Cache...');

  try {
    // Dynamic import for response cache plugin
    ...
    // Create Yoga server with response cache
    const yoga = createYoga({
      schema: gatewaySchema,
      maskedErrors: false,
      graphiql: {
        title: 'Advanced Mesh Gateway with Cache',
    ...
    activeSessions {
      ...
      plugins: [
        ...
        useResponseCache({
          ...
          // Invalidate cache when mutations are executed
          invalidateViaMutation: true,
          
          // Include cache metadata in response
          includeExtensionMetadata: true,
          
          // Session function for cache key generation
          session: () => null,
          
          // TTL per GraphQL type
          ttlPerType: {
            SystemHealth: 5_000,     // 5 seconds for health checks
            Repo_GitStatus: 30_000,  // 30 seconds for git status
          },
          
          // TTL per specific field
          ttlPerSchemaCoordinate: {
            'Query.systemHealth': 5_000,
            'Query.repositoryWithAnalysis': 10_000,
          }
        })
      ]
    });
