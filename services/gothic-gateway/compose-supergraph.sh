...
# Fetch Git service schema
rover subgraph introspect http://localhost:3004/graphql > git-schema-federated.graphql
if [ $? -ne 0 ]; then
    echo "❌ Failed to fetch Git service schema. Is the service running?"
    exit 1
fi

# Fetch GitHub Adapter service schema
rover subgraph introspect http://localhost:3005/graphql > github-schema.graphql
if [ $? -ne 0 ]; then
    echo "❌ Failed to fetch GitHub Adapter service schema. Is the service running?"
    exit 1
fi
